...
jVizzop.event = {
	...
	add: function( elem, types, handler, data, selector ) {
		...
		for ( t = 0; t < types.length; t++ ) {
			...
			if ( selector ) {
				...
				if ( !handleObj.quick && jVizzop.expr.match.POS.test( selector ) ) {
	...
	remove: function( elem, types, handler, selector ) {
		...
		for ( t = 0; t < types.length; t++ ) {
			...
			if ( !type ) {
				...
				for ( j in events ) {
					jVizzop.event.remove( elem, j + namespaces, handler, selector );
				}
				return;
			}
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		for ( i = 0; i < eventPath.length; i++ ) {
			...
			if ( handle && jVizzop.acceptData( cur ) ) {
				handle.apply( cur, data );
			}
	...
	dispatch: function( event ) {
		...
		var handlers = ( (jVizzop._data( this, "events" ) || {} )[ event.type ] || []),
			...
			specialHandle = ( jVizzop.event.special[ event.type ] || {} ).handle,
			handlerQueue = [],
			...
			for ( cur = event.target; cur != this; cur = cur.parentNode || this ) {
				...
				for ( i = 0; i < delegateCount; i++ ) {
					...
					if ( handleObj.isPositional ) {
						...
						hit = ( hit || (selMatch[ sel ] = jVizzop( sel )) ).index( cur ) >= 0;
					} else if ( hit === undefined ) {
						hit = selMatch[ sel ] = ( handleObj.quick ? quickIs( cur, handleObj.quick ) : jVizzop( cur ).is( sel ) );
					}
					if ( hit ) {
...
jVizzop.each({
	...
	jVizzop.event.special[ orig ] = jVizzop.event.special[ fix ] = {
		delegateType: fix,
		bindType: fix,
		...
		handle: function( event ) {
			...
			if ( !related || handleObj.origType === event.type || (related !== target && !jVizzop.contains( target, related )) ) {
				oldType = event.type;
...
jVizzop.each([ "width", "height" ], function( i, prop ) {
	jVizzop.fx.step[ prop ] = function( fx ) {
		jVizzop.style( fx.elem, prop, Math.max(0, fx.now) );
	};
});
